...
REST·SOAP·MQ·배치·SFTP 5종 이기종 인터페이스를 **단일 화면에서 등록·실행·모니터링·재처리**하는 중앙 관리 플랫폼 프로토타입

![Dashboard](docs/screenshots/readme/readme-02-dashboard-hero.png)

`Java 17` · `Spring Boot 3.3.5` · `Vue 3 + Vuetify 3` · `PostgreSQL 16` · `Server-Sent Events` · `Docker Compose` · **자동 테스트 45 PASS** · **1주 (Day 1~8)**
...
8. [개발 과정 (Day 1~8)](#8-개발-과정-day-18)
...
3 컨테이너 healthy 상태 예시 ([전체 출력](docs/screenshots/day8-docker-compose-ps.txt)):
...
대시보드 구성:
- **4 카드** — 전체 / 실행중 / 성공 / 실패. 실패 카드 클릭 → `/history?status=FAILED` 드릴다운 (Day 7 M5)
...
- **ID 34 SCHEDULER RUNNING** — CRON 인터페이스가 자동 발화한 최신 실행 (Day 8)
...
- URL 쿼리 `?status=FAILED` 로 필터 상태 보존 (뒤로가기/북마크 호환, Day 7 M5)
...
2. **CRON 자동 실행 (Day 8)**: `InterfaceCronScheduler` 가 1분 주기 폴링 → ACTIVE+CRON 인터페이스 조회 → `CronExpression.next(lastScheduledAt) <= now` 인 것만 `ExecutionTriggerService.trigger(SCHEDULER, ...)` 호출. 기존 advisory lock + `uk_log_running` 보호막을 그대로 재사용.
...
본 프로토타입은 단순히 CRUD를 쌓지 않고, 금융 SI 운영 현실에서 드러나는 동시성·감사·실패 모드를 **Architecture Decision Records** 7건으로 명문화했다.
...
- **Day 8 파급**: CRON 스케줄러와 수동 실행이 동시에 같은 인터페이스를 트리거해도 **정확히 1건만 성공** (ConflictException 흡수). E2E 로 실측 확인.
...
### Day 8 추가 판단 (본 README 제출용)

- **CRON 자동 실행 실구현** — planning §3에 명시됐던 필수 기능 완성. `catch-up 안 함`을 명시적 설계로 채택 (재기동 시 `last_scheduled_at=NULL` → 첫 tick을 기준점으로만 기록).
- **JWT 전환 거부** — 4-에이전트 회의(Security/DBA/DevilsAdvocate) 전원 반대 수렴. 세션+CSRF+HttpOnly 쿠키가 금융권 표준에 더 가깝고, 1일 내 완전 전환 불가 + 기존 E2E 회귀 리스크. 근거는 `docs/superpowers/specs/2026-04-22-day8-submission-polish-design.md` §1.
- **@Transactional self-invocation 회귀** — Task 3 단위 테스트 5건 + 리뷰 2종 전부 통과했으나, 실 Docker E2E에서 `sweep() → this.tick()` 자기호출이 프록시 우회해 `@Transactional` 미적용인 것을 발견. `configRepository.save()` 명시 호출 + 회귀 테스트 4건 추가로 fix. 상세는 [`DAY8-SUMMARY §5`](docs/DAY8-SUMMARY.md).
...
./gradlew test --tests "*InterfaceCronSchedulerTest*"       # Day 8 CRON 5 케이스
...
| Day 6 SSE·delta·cursor·rate | 17 | `DeltaServiceTest` · `CursorTest` · `RateLimiterTest` · `SseEmitterServiceTest` |
| Day 7 RetryGuard 정책 | 8 | ADR-005 Q1/Q2 × SYSTEM/ANONYMOUS/truncated/inactive/not_leaf/all_pass |
| Day 7 SnapshotParity / ApiResponse / Masking bench | 4 | Detail↔Snapshot 필드 정합 · ALWAYS 직렬화 · p95 벤치 |
| Day 7 SSE subscribe race (`@RepeatedTest(20)`) | 1 | M4 재현용. 현재 @Disabled (fix 후 활성) |
| **Day 8 InterfaceCronScheduler** | **5** | 발화 / 미발화 / 최초기동 / 경합흡수 / 잘못된cron |
...
| Day | 핵심 산출 | 문서 |
|---|---|---|
| Day 2 | 도메인 모델 · JPA · 인터페이스 CRUD API | [DAY2-SUMMARY](docs/DAY2-SUMMARY.md) |
| Day 3 | 실행 트리거 · Mock 실행기 5종 · RetryService · Watchdog | [DAY3-SUMMARY](docs/DAY3-SUMMARY.md) |
| Day 4 | Spring Security · SSE 기반 · 대시보드 집계 · ConnectionLimit | [DAY4-SUMMARY](docs/DAY4-SUMMARY.md) |
| Day 5 | Vue 3 scaffold · InterfaceList/Form · 낙관적 락 충돌 처리 | [DAY5-SUMMARY](docs/DAY5-SUMMARY.md) |
| Day 6 | Dashboard · ExecutionHistory · delta API · ADR-007 | [DAY6-SUMMARY](docs/DAY6-SUMMARY.md) |
| Day 7 | M1~M9 부채 청산 · 자동 테스트 12건 + race · 문서 정합 | [DAY7-SUMMARY](docs/DAY7-SUMMARY.md) |
| **Day 8** | **CRON 자동 실행 + Docker 한 줄 기동 + E2E Playwright 자동화** | **[DAY8-SUMMARY](docs/DAY8-SUMMARY.md)** |
...
```
Day 1  기획·ERD·API 명세·프로젝트 초기화
Day 2  도메인·JPA·인터페이스 CRUD              (ADR-001, ADR-004, ADR-006)
Day 3  실행 트리거·Mock 실행기·재처리·Watchdog
Day 4  Security·SSE·대시보드·ArchUnit·traceId
Day 5  Vue 프런트 scaffold·목록·등록·수동 실행
Day 6  이력·대시보드·SSE 연동·delta             (ADR-007)
Day 7  부채 청산·통합/단위/벤치 테스트·문서 정합
Day 8  CRON 자동 실행·Docker 한 줄 기동·E2E 자동화  ← 본 README 제출 시점
```

### 방법론

- **4-에이전트 회의** (Architect / Security / DBA / DevilsAdvocate) — 결정 시점마다 ADR 또는 spec 본문 편입
- **subagent-driven development** (Day 8) — 9 Task 분할 후 각 Task마다 implementer + spec-review + code-quality-review 3 subagent 파이프라인 적용. Important/Critical 이슈 9건 발견→fix 루프
- **Playwright MCP E2E 자동화** (Day 8) — 수동 검증을 브라우저 자동화로 치환. 본 README 의 8개 스크린샷 모두 자동 촬영
...
| 제외 항목 | 현재 상태 | 이관 트리거 |
|---|---|---|
| JWT 인증 / SSO | 세션+CSRF+HttpOnly 쿠키. Day 8 회의에서 JWT 전환 거부 수렴 | Role 분리와 함께 재검토 |
